...
// - Long-press of the reset button reopens registration without forgetting peers
//
...
const unsigned long RESET_HOLD_THRESHOLD_MS = 1000;  // Hold time (ms) to trigger registration search.
bool resetPressActive = false;                // Track whether the reset button is currently held.
bool resetHoldFired = false;                  // Remember if the long-hold event already fired for this press.
bool resetHoldActive = false;                 // Flag while the long-hold mode is engaged.
unsigned long resetPressStartMs = 0;          // Timestamp marking when the latest press began.
...
ResetEvent pollResetButton() {                     // Debounce the reset pin and report tap/hold events.
  ...
  if ((millis() - resetLastDebounceMs) > RESET_DEBOUNCE_MS) {   // After the value is stable long enough...
    if (reading != resetStableState) {                          // ...and it differs from the last stable state...
      ...
      if (resetStableState == RESET_PRESSED_LEVEL) {            // The button just transitioned into "pressed".
        resetPressActive = true;                                // Remember that the button is currently held down.
        resetHoldFired = false;                                 // Clear any prior hold event for this press.
        resetHoldActive = false;                                // Not yet in hold mode until the timer elapses.
        resetPressStartMs = millis();                           // Record when the press began.
      } else {                                                  // The button transitioned into "released".
        if (resetPressActive) {                                 // Only act if we previously saw a press.
          unsigned long heldMs = millis() - resetPressStartMs;  // Calculate how long the press lasted.
          bool firedHold = resetHoldFired;                      // Remember whether the hold already triggered.
          resetPressActive = false;                             // The button is no longer held.
          resetHoldActive = false;                              // Stop extending the registration window.
          resetHoldFired = false;                               // Clear the hold flag for the next press.
          if (!firedHold && heldMs < RESET_HOLD_THRESHOLD_MS) { // Short press: no hold event triggered earlier.
            return RESET_EVENT_TAP;                             // Report a tap event to the caller.
          }
        }
      }
    }
  ...
  if (resetPressActive && !resetHoldFired) {                    // Button is still held and hold not fired yet.
    if ((millis() - resetPressStartMs) >= RESET_HOLD_THRESHOLD_MS) {  // Hold threshold reached.
      resetHoldFired = true;                                    // Prevent retriggering during this press.
      resetHoldActive = true;                                   // Mark that the hold mode is currently active.
      return RESET_EVENT_HOLD;                                  // Report the long-hold event immediately.
    }
  }

  return RESET_EVENT_NONE;                      // No new event this iteration.
}  // End of pollResetButton.

void handleShortResetPress() {                   // Respond to a quick tap of the reset button.
  Serial.println("[Hub] Reset button tapped.");  // Log that we received a short press.
...
}  // End of handleShortResetPress.

void handleResetHold() {                          // Respond when the reset button is held past the threshold.
  Serial.println("[Hub] Reset button held; reopening registration without clearing peers.");  // Log the hold behavior.
  expected_id = 1;                                // Reset the sequence expectation.
  counted = 0;                                    // Clear FFA progress.
  resetRoundRuntime();                            // Reset per-peer round state.
  broadcastRoundReset();                          // Ensure every button unlocks for the refreshed round.

  mode = MODE_IDLE;                               // Return to idle while we gather additional registrations.
  reg_open = true;                                // Explicitly open the registration window.
  reg_deadline_ms = millis() + REG_WINDOW_MS;     // Give operators a fresh window to add more buttons.
  participants = countPresent();                  // Snapshot how many peers are currently known.
  rebuildIdMap();                                 // Keep the ID map in sync for diagnostics.
}  // End of handleResetHold.
...
void loop() {                                                       // Main loop runs repeatedly after setup.
  ...
  ResetEvent resetEvent = pollResetButton();                       // Poll the reset button for tap/hold events.
  if (resetEvent == RESET_EVENT_TAP) {                             // Short press detected this iteration.
    handleShortResetPress();                                       // Reset the round using existing registrations.
  } else if (resetEvent == RESET_EVENT_HOLD) {                     // Long hold detected.
    handleResetHold();                                             // Reopen registration without clearing peers.
  }

  if (resetHoldActive && reg_open) {                               // While the button stays held in hold mode...
    reg_deadline_ms = millis() + REG_WINDOW_MS;                    // ...keep extending the registration window.
  }
