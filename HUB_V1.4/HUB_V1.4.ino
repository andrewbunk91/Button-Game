// Version 1.4 — Struct order fix for Arduino prototype generation issues.
// ================== HUB_V1.4.ino ==================
...
// NOTE: Maintain the struct grouping above any functions so Arduino's
// autogenerated prototypes see the custom types first.
...
void addEspNowPeerIfNeeded(const uint8_t mac[6]) {
  ...
  p.encrypt = false;
  esp_err_t status = esp_now_add_peer(&p);
  if (status != ESP_OK && status != ESP_ERR_ESPNOW_EXIST) {
    printScreen("[Hub] Failed to add ESP-NOW peer.");
...
void setup() {
  ...
  if (chanStatus != ESP_OK) {
    printScreen("[Hub] Failed to set ESP-NOW channel.");
    Serial.printf("ESP-NOW error: %d\n", chanStatus);
  }
